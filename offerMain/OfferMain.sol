...
contract OfferMain is AccessController {
    ...
    function _initInstanceVariables() internal override {
        ...
        _priceDataContract = IPriceData(getContractAddress("priceData"));
    ...
    function offer(uint256 ethAmount, uint256 erc20Amount, address erc20) public payable onlyEOA {
        require(_tokenPairData.isValidToken(erc20), "offerMain: token not allowed");
        require(_tokenPairData.isHtToken(erc20), "offerMain: token type error");
        ...
        require(msg.value >= ethAmount, "offerMain: eth value should greater than ethAmount");
        uint256 change = msg.value.sub(ethAmount);

        if (change > 0)
            _payEth(msg.sender, change);
    ...
    function _ming(address token) private {
        ...
        if (_tokenPairData.isDefaultToken(token)) {
            miningAmount = _mingContract.mining(0);
            if (miningAmount > 0) {
    ...
    function sendEthBuyErc20(uint256 ethAmount, uint256 erc20Amount, address offerIndex, uint256 tradeEthAmount, uint256 tradeErc20Amount, address erc20) public payable onlyEOA {
        _tradeOffer(TradeChoices.SendEthBuyERC20, ethAmount, erc20Amount, offerIndex, tradeEthAmount, tradeErc20Amount, erc20);
    }

    function sendErc20BuyEth(uint256 ethAmount, uint256 erc20Amount, address offerIndex, uint256 tradeEthAmount, uint256 tradeErc20Amount, address erc20) public payable onlyEOA {
    ...
    function _tradeOffer(TradeChoices tradeChoices, uint256 ethAmount, uint256 erc20Amount, address offerIndex, uint256 tradeEthAmount, uint256 tradeErc20Amount, address erc20) private {
        ...
        if (tradeChoices == TradeChoices.SendEthBuyERC20) {
            require(msg.value == ethAmount.add(tradeEthAmount), "offerMain: msg.value needs to be equal to the quotation eth quantity plus transaction eth");
        } else {
            require(msg.value == ethAmount.sub(tradeEthAmount), "offerMain: msg.value needs to be equal to the quoted eth quantity sub transaction eth");
        }
        ...
        if(tradeOffer.miningFee != 0){
            if(_isAllArbitrage(index)){
                ...
                uint invalidFee = leastEth.mul(tradeEthAmount).div(originalEth);
    ...
    function withdraw(address offerIndex) public onlyEOA {
        ...
        if (targetOffer.ethAmount > 0) {
            ...
            _payEth(targetOffer.owner, ethAmount);
        }
        ...
        if (targetOffer.miningFee > 0) {
            ...
            if (_tokenPairData.isDefaultToken(targetOffer.token)) {
                ...
                if(_isReturnMiningOfBlock[targetOffer.blockNum][targetOffer.token]){
                    uint256 takeMining=mining.mul(_takeFeeOfBlock[targetOffer.blockNum][targetOffer.token]).div(feeOfBlock[targetOffer.blockNum][targetOffer.token]);
                    _mingContract.depositHomerInner(0,mining.sub(takeMining));
    ...
    function _payEth(address account, uint256 ethAmount) private {
        address payable addr = Address.make_payable(account);
        Address.sendValue(addr, ethAmount);
    }
